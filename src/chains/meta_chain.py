...
2. **スタッツに基づく構成比較・立ち位置の解説（必須）**
    - 単に並べるのではなく、1位率・平均順位・Top4率を根拠に、強み・リスクの違いを明確に比較してください。
    - 「上限完成時」「標準進行時」「完成前」などの状態別評価は生成禁止です。提供データには状態別の統計がありません。
    - 「上限」や「最高火力」と表現する場合も、実際の完成状態の測定値ではなく、1位率が高いことに基づく「統計上の1位性能」と明記してください。
    - 分類の根拠は固定です。1位率12.5%以下=Top4狙い、12.5%超20%未満=バランス型、1位率20%以上=1位狙い、1位率20%以上かつ平均順位4.5位以上=ハイリスク・ハイリターンです。
...
2. **分類根拠の提示**（1位率・平均順位・Top4率に基づく単一分類）
    - 状態別の統計データは存在しないため、「上限完成時」「標準進行時」などの別集計を作らないこと。
3. **メインキャリー・メインタンクの推奨アイテム（BIS）**
